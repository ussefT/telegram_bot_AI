...
async function testModel(env, user) {
  if (user.useDefault || (!user.baseUrl && !user.apiKey)) {
    if (!env.AI) return { ok: false, detail: 'no AI binding' };
    try { await env.AI.run(DEFAULT_CF_MODEL, { messages: [{ role: 'user', content: 'Say OK' }] }); return { ok: true, detail: 'default CF model answers' }; }
    catch (e) { console.error('default test:', e.message); return { ok: false, detail: e.message.slice(0, 200) }; }
  }
  if (!user.baseUrl || !user.apiKey || !user.model) return { ok: false, detail: 'incomplete setup' };
  try {
    const r = isAnth(user)
      ? await fetch(user.baseUrl + '/messages', { method: 'POST', headers: anthHeaders(user),
          body: JSON.stringify({ model: user.model, max_tokens: 10, messages: [{ role: 'user', content: 'Say OK' }] }) })
      : await fetch(user.baseUrl + '/chat/completions', {
          ...
          body: JSON.stringify({ model: user.model, messages: [{ role: 'user', content: 'Say OK' }], max_tokens: 5 }),
        });
    const t = await r.text();
    if (!r.ok) { console.error('model test:', r.status, t.slice(0, 500)); return { ok: false, detail: `HTTP ${r.status}: ${t.slice(0, 200)}` }; }
    return { ok: true, detail: 'model answers' };
  } catch (e) { console.error('model test fetch:', e.message); return { ok: false, detail: e.message.slice(0, 200) }; }
}
...
async function chat(env, token, chatId, msgId, user, text) {
  ...
  try {
    ...
    // Counters: per-user message # + cumulative tokens
    user.msgs = (user.msgs || 0) + 1;
    const p = usage?.prompt_tokens || 0, c = usage?.completion_tokens || 0, t = usage?.total_tokens ?? (p + c);
    ...
    const footer = `\n\n<i>────────</i>\n<i>📩 #${user.msgs} • 🤖 <code>${esc(modelName)}</code>\n🎟️ in ${p} • out ${c} • total ${t} this reply\n📊 total in ${user.tokens.p} • out ${user.tokens.c} • total ${user.tokens.t}</i>`;
