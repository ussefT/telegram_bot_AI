...
export default {
    async fetch(request, env, ctx) {
        if (request.method === "GET") {
            ...
            // ponytail: one-tap activation — open <worker>/activate to set webhook + commands
            if (url.pathname === "/activate") {
                if (!env.BOT_TOKEN) return new Response("Missing BOT_TOKEN binding.", { status: 500 });
                try {
                    const webhookUrl = url.origin + "/";
                    const sw = await fetch(telegramApiUrl(env, "setWebhook"), {
                        method: "POST",
                    ...
                    if (!sw || sw.ok === false) {
                        console.error("activate setWebhook failed:", JSON.stringify(sw));
                        return new Response("Activate failed: " + JSON.stringify(sw).slice(0, 500), { status: 500 });
                    }
                    return new Response("✅ Bot activated for " + webhookUrl + ". Open Telegram and send /start", { status: 200 });
                } catch (err) {
                    console.error("activate error:", err);
                    return new Response("Activate failed: " + err.message, { status: 500 });
                }
            }
            return new Response(
                "Telegram Bot Worker (worker_cloud.js) is running!\n\n" +
                `Webhook URL: ${url.origin}/\n\n` +
                "To activate (set webhook + commands), open:\n" +
                `${url.origin}/activate\n\n` +
                "Or manually:\n" +
...
async function handleChat(userText, chatId, userId, replyToId, env) {
    ...
    let reply;
    try {
        const result = await withTimeout(env.AI.run(model, { messages, max_tokens: MAX_OUTPUT_TOKENS }), 55000, "AI chat");
        ...
        while (looksTruncated(reply) && continuations < MAX_CONTINUATIONS) {
            ...
            const continueResult = await withTimeout(env.AI.run(model, {
                messages: continueMessages,
                max_tokens: MAX_OUTPUT_TOKENS,
            }), 55000, "AI continuation");
...
function looksTruncated(text) {
    ...
    // ponytail: timeout guard so a hung AI/Telegram call replaces the
    // placeholder with "not working" instead of leaving Thinking… stuck.
    ...
    async function sendMarkdownSafe(env, chatId, text, extraOptions) {
        ...
        for (const chunk of chunks) {
            ...
            try {
                ...
                : "";
                // ponytail: never drop a chunk — parse errors and transport failures fall back to plain text
                if (!response || description.toLowerCase().includes("parse")) {
                    console.log("Markdown send failed, falling back to plain text for:", chunk.slice(0, 80));
    ...
    async function handleImage(prompt, chatId, replyToId, env) {
        ...
        try {
            const result = await withTimeout(env.AI.run(IMAGE_MODEL, { prompt }), 55000, "AI image");
    ...
    async function handleTranscribe(message, chatId, replyToId, env) {
        ...
        try {
            ...
            const result = await withTimeout(env.AI.run(WHISPER_MODEL, input), 55000, "AI transcription");
    ...
    async function sendMessage(env, chatId, text, extraOptions) {
        try {
            ...
            const response = await fetchWithTimeout(telegramApiUrl(env, "sendMessage"), {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    async function editMessageText(env, chatId, messageId, text, options) {
        ...
        try {
            ...
            const response = await fetchWithTimeout(telegramApiUrl(env, "editMessageText"), {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(body),
            }, 15000);
    ...
    async function getFile(fileId, env) {
        const response = await fetchWithTimeout(`${telegramApiUrl(env, "getFile")}?file_id=${encodeURIComponent(fileId)}`, {}, 15000);
        if (!response.ok) {
            const errText = await safeText(response);
